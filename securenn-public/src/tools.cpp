...
{
    cout << "Usage: " << bin << " MODEL_NAME NUM_EPOCHS MPC_TYPE PARTY_NUM IP_ADDR_FILE AES_SEED_INDEP AES_SEED_COMMON TRAINING_DATA TRAINING_LABELS [OUTPUT_FILE]" << endl;
    cout << endl;
    cout << "Required Arguments:\n";
    cout << "MODEL_NAME   Model to use for training (e.g. SimpleNN)\n";
    cout << "NUM_EPOCHS   Number of epochs to train the model for\n";
    ...
    //cout << "AES_KEY_DIR\t\t Directory that contains the needed keys for computation\n";
    ...
    cout << "OUTPUT_FILE\t\tFile name to output model to\n";
    cout << endl;
    exit(-1);
}
...
void end_time(string str)
{
	...
	cout << "------------------------------------" << endl;
	cout << "Wall Clock time for P" << partyNum << " " << str << ": " << diff(requestStart, requestEnd) << " sec\n";
	cout << "CPU time for P" << partyNum << " " << str << ": " << (double)(clock() - tStart)/CLOCKS_PER_SEC << " sec\n";
	cout << "------------------------------------" << endl;	
...
{
  Map<Matrix<myType, Dynamic, Dynamic,RowMajor>> eigen_c(c.data(), rows, columns);

  if(transpose_a && transpose_b) {
    Map<Matrix<myType, Dynamic, Dynamic>> eigen_a(a.data(), rows, common_dim);
    Map<Matrix<myType, Dynamic, Dynamic>> eigen_b(b.data(), common_dim, columns);
    eigen_c = eigen_a * eigen_b;
  } else if (transpose_a) {
    Map<Matrix<myType, Dynamic, Dynamic>> eigen_a(a.data(), rows, common_dim);
    Map<Matrix<myType, Dynamic, Dynamic,RowMajor>> eigen_b(b.data(), common_dim, columns);
    eigen_c = eigen_a * eigen_b;
  } else if (transpose_b) {
    Map<Matrix<myType, Dynamic, Dynamic,RowMajor>> eigen_a(a.data(), rows, common_dim);
    Map<Matrix<myType, Dynamic, Dynamic>> eigen_b(b.data(), common_dim, columns);
    eigen_c = eigen_a * eigen_b;
  } else {
    Map<Matrix<myType, Dynamic, Dynamic,RowMajor>> eigen_a(a.data(), rows, common_dim);
...
{
	assert((b != 0) && "Cannot divide by 0");
	auto val = static_cast<int64_t>(a)/static_cast<int64_t>(b);
  return (val < 0) ? -(myType)(-val) : (myType)val;
...
void dividePlainSA(vector<myType> &vec, int divisor)
{
    ...
    auto val = static_cast<int64_t>(vec[i])/static_cast<int64_t>(divisor);
    vec[i] = (val < 0) ? -(myType)(-val) : (myType)val;
  }
}
...
size_t adversary(size_t party)
{
    ...
    case PARTY_B : ret = PARTY_A;
             break;
    default:
             assert(false && "Adversary should only be called for parties A/B");
             break;
	}	
	return ret;
}
...
void outputTensor(ofstream& out, const vector<myType>& params, vector<size_t> shape) {

  out << "[ ";
  if(shape.size() == 1) {
    auto cols = shape[0];
    assert(params.size() >= cols);

    out << myTypeToFloat(params[0]);
    for(size_t i = 1; i < cols; ++i) {
      out << ", " << myTypeToFloat(params[i]);
    }
  }
  else if(shape.size() == 2) {
    auto rows = shape[0];
    auto cols = shape[1];
    assert(params.size() >= rows*cols);

    for(size_t i = 0; i < rows; ++i) {
      out << "[ " << myTypeToFloat(params[i*cols]);
      for(size_t j = 1; j < cols; ++j) {
        out << ", " << myTypeToFloat(params[i*cols + j]);
      }
      out << " ]";
      if(i + 1 != rows) {
        out << ", ";
    ...
    assert(false && "outputting only supports 1,2 dimensional tensors");
  }
  out << " ]";
}
