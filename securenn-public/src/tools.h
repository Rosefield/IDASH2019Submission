...
{	
  ...
  for(size_t i = 0; i < size; ++i) {
    ...
    if(r_type == "COMMON") {
        ...
        aes = aes_indep;
    } else if (neg_type == "NEGATIVE" || sizeof(T) != sizeof(myType)) {
        assert(false && "the ac/bc common randomness is positive only and only for myType");
    } else if (r_type == "a_1") {
        aes = aes_a_1;
    } else if (r_type == "a_2") {
        aes = aes_a_2;
        ...
        assert(false && "unxpected combination of parameters");
    }

	myType sign = 1;
    if (neg_type == "NEGATIVE" && (partyNum == PARTY_B)) {
        sign = MINUS_ONE;
    }
    ...
    {
        for (size_t i = 0; i < size; ++i)
            vec[i] = sign*aes->get64Bits();		
    }
    else
    {
        for (size_t i = 0; i < size; ++i)
            vec[i] = sign*aes->get8Bits();		
    }
}
